import os, re, uuid, json, time, shutil, zipfile, threading, tempfile, logging
...
# ── in-memory job store ───────────────────────────────────────────────────────
jobs: dict[str, dict] = {}          # job_id → {queue, status, thread}
        ...
        for idx, url in enumerate(urls, 1):
            ...
            try:
                ...
                emit('progress', {
                    ...
                    'status': 'failed', 'message': f'Download failed: {e}'
                })
                fail += 1; continue
            ...
            emit('progress', {
                'index': idx, 'total': total, 'url': short,
                'status': 'watermarking', 'message': f'Adding logo to reel {idx}/{total}…'
            })
...
def index():
    ...
    return render_template('index.html', has_logo=has_logo)
...
def upload_logo():
    ...
    img.save(LOGO_PATH)
    return jsonify(ok=True, message='Logo saved successfully')


@app.route('/start', methods=['POST'])
def start():
    if not LOGO_PATH.exists():
        return jsonify(error='No logo uploaded yet. Please upload a logo first.'), 400
    ...
    q    = Queue()
    jobs[job_id] = {'queue': q, 'status': 'running', 'events': []}

    t = threading.Thread(target=process_job, args=(job_id, xls_path, logo), daemon=True)
    t.start()
...
def stream(job_id):
    ...
    def generate():
        q    = jobs[job_id]['queue']
        sent = 0
        # replay already-emitted events for late SSE connects
        while True:
            existing = jobs[job_id]['events']
            while sent < len(existing):
                ev = existing[sent]
                yield f"event: {ev['event']}\ndata: {json.dumps(ev['data'])}\n\n"
                sent += 1
            if jobs[job_id]['status'] in ('done', 'error') and sent >= len(existing):
                break
            try:
                ev = q.get(timeout=1)
                yield f"event: {ev['event']}\ndata: {json.dumps(ev['data'])}\n\n"
